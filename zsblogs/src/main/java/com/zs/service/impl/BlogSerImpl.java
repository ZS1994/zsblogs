...
public class BlogSerImpl implements BlogSer {
    ...
    public EasyUIPage queryFenye(EasyUIAccept accept) {
        if (accept != null) {
            ...
            // 张顺，2023-11-14 16:04:42，如果str3包含，则转为list并存储到intList3中，把int3改为null
            if (StrUtil.isNotEmpty(accept.getStr3()) && accept.getStr3().indexOf(",") > -1) {
                List<String> strList3 = StrUtil.split(accept.getStr3(), ",")
                        .stream().filter(s -> StrUtil.isNotEmpty(s)).collect(Collectors.toList());
                List<Integer> intList3 = strList3.stream().map(s -> Integer.parseInt(s)).collect(Collectors.toList());
                accept.setInt3(null);
                accept.setIntList3(intList3);
            }
            // str5代表年份，如果str5不为空，则填充时间范围
            if (StrUtil.isNotEmpty(accept.getStr5())) {
                Integer year = Integer.parseInt(accept.getStr5());
                // 填充时间
                String dataStr1 = StrUtil.format("{}-00-00 00:00:00", year);
                String dataStr2 = StrUtil.format("{}-12-31 23:59:59", year);
                Date date1 = DateUtil.parseDateTime(dataStr1);
                Date date2 = DateUtil.parseDateTime(dataStr2);
                accept.setDate1(date1);
                accept.setDate2(date2);
            }
